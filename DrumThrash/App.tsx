...
import { MusicKey, Scale, DrumPattern, JamState, SynthPreset, ArpeggiatorRate, ArpeggiatorDirection, HarmonyInterval, CustomDrumPattern, DrumStep } from './types';
import { audioService, AudioService } from './services/audioService';
import { saveJam, loadJam } from './services/jamService';
...
const App: React.FC = () => {
  ...
  const [kickVolume, setKickVolume] = useState<number>(-6);
  const [snareVolume, setSnareVolume] = useState<number>(-6);
  const [hihatVolume, setHihatVolume] = useState<number>(-12);
  const [synthVolume, setSynthVolume] = useState<number>(-9);
  const [bassVolume, setBassVolume] = useState<number>(-6);
  ...
  const [synthPreset, setSynthPreset] = useState<SynthPreset>('Sawtooth');
  ...
  const [synthFilterResonance, setSynthFilterResonance] = useState<number>(1);
  ...
  const [harmonyVolume, setHarmonyVolume] = useState<number>(-18);
  ...
  }, [selectedPresetIndex, musicKey, scale]);
  ...
  const getCurrentJamState = useCallback((): JamState => ({
    ...
    synthVolume, bassVolume, selectedPresetIndex, synthPreset, useInversions, synthOctave, synthFilterCutoff, synthFilterResonance,
    voicingVariation, spreadVoicing, harmonyInterval, harmonyVolume,
    arpeggiatorEnabled, arpeggiatorRate, arpeggiatorDirection, arpeggiatorGate,
    kickPan, snarePan, hihatPan, synthPan,
    customDrumPatterns,
  }), [
    progression, bpm, musicKey, scale, drumPattern, kickVolume, snareVolume, hihatVolume,
    synthVolume, bassVolume, selectedPresetIndex, synthPreset, useInversions, synthOctave, synthFilterCutoff, synthFilterResonance,
    voicingVariation, spreadVoicing, harmonyInterval, harmonyVolume, arpeggiatorEnabled, arpeggiatorRate,
    arpeggiatorDirection, arpeggiatorGate, kickPan, snarePan, hihatPan, synthPan, customDrumPatterns
  ]);
  ...
  const handlePlayToggle = useCallback(async () => {
    ...
    try {
      ...
      setError(`Audio-Engine konnte nicht gestartet werden. Bitte klicke auf die Seite und versuche es erneut: ${err.message}`);
      setIsPlaying(false);
    }
  }, [isPlaying, progression, drumPattern, resolveDrumPattern, audioState]);
  ...
  useEffect(() => {
      ...
      audioService.setSynthPreset(synthPreset);
      audioService.setUseInversions(useInversions);
      audioService.setSynthOctave(synthOctave);
      audioService.setSynthFilterCutoff(synthFilterCutoff);
      audioService.setSynthFilterResonance(synthFilterResonance);
      ...
      bpm, kickVolume, snareVolume, hihatVolume, synthVolume, bassVolume, 
      kickPan, snarePan, hihatPan, synthPan,
      synthPreset, useInversions, 
      synthOctave, synthFilterCutoff, synthFilterResonance, voicingVariation, spreadVoicing, harmonyInterval, harmonyVolume,
      arpeggiatorEnabled, arpeggiatorRate, arpeggiatorDirection, arpeggiatorGate
  ]);
  ...
  }, [isPlaying, progression, drumPattern, synthPreset, resolveDrumPattern]);
  ...
  const handleGenerateWithAi = useCallback(async (prompt: string) => {
    ...
    clearError();
    try {
      const result = await generateProgression(prompt);
      setMusicKey(result.key);
      setScale(result.scale);
      setProgression(result.progression);
      setSelectedPresetIndex(-1); // It's a custom progression now
    } catch (err: any) {
      setError(`KI-Fehler: ${err.message}`);
      setRetryAction(() => () => handleGenerateWithAi(prompt));
    } finally {
      setIsLoading(false);
  ...
  const handleLoad = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setSynthPreset(loadedState.synthPreset);
      setUseInversions(loadedState.useInversions);
      setSynthOctave(loadedState.synthOctave);
      setSynthFilterCutoff(loadedState.synthFilterCutoff);
      setSynthFilterResonance(loadedState.synthFilterResonance);
  ...
  if (audioState !== 'ready') {
    return (
      <div className="fixed inset-0 bg-gray-900 z-50 flex flex-col items-center justify-center text-center p-4">
        ...
        {audioState === 'error' && (
           <div className="bg-red-500/20 border border-red-500 text-red-300 px-4 py-3 rounded-lg relative max-w-xl" role="alert">
  ...
  return (
    <div className="bg-gray-900 text-white min-h-screen font-sans">
      <header className="p-4 text-center border-b border-gray-800">
        ...
        <p className="text-gray-400">Erstelle und spiele Akkordfolgen mit Leichtigkeit</p>
      </header>
      ...
      <main className="container mx-auto p-4 md:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 bg-gray-800/50 p-6 rounded-2xl shadow-lg border border-gray-700 relative">
            <ControlPanel
              musicKey={musicKey}
              scale={scale}
              ...
              onSynthPresetChange={setSynthPreset}
              ...
              synthFilterCutoff={synthFilterCutoff}
              onSynthFilterCutoffChange={setSynthFilterCutoff}
              synthFilterResonance={synthFilterResonance}
              onSynthFilterResonanceChange={setSynthFilterResonance}
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-gray-800/50 p-6 rounded-2xl shadow-lg border border-gray-700 relative">
            ...
            <div className="bg-gray-800/50 rounded-2xl shadow-lg border border-gray-700">
                <CircleOfFifths 
                    progression={progression}
                    musicKey={musicKey}
                    scale={scale}
      ...
      {showAiGenerator && <AiGenerator onClose={() => setShowAiGenerator(false)} onGenerate={handleGenerateWithAi} />}
      {showDrumEditor && (
        <DrumPatternEditor 
