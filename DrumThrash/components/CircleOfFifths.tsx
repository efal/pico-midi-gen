    ...
    <div role="region" aria-labelledby="cof-title">
      ...
      <div className="p-4">
        <svg viewBox={`0 0 ${size} ${size}`} className="w-full h-auto" aria-label="Quintenzirkel-Diagramm, das die aktuelle Progression anzeigt">
          {/* Background rings */}
          <circle cx={center} cy={center} r={majorRadius * 1.05} fill="none" stroke="#4a5568" strokeWidth="1" />
          <circle cx={center} cy={center} r={minorRadius * 1.15} fill="none" stroke="#4a5568" strokeWidth="1" />
          ...
          {activeChordTonesPoints.map(({ x, y, tone }, index) => {
            ...
            const color = isThird ? '#f87171' : '#1ddaff'; // red for third, cyan for others
            const colorRgba = isThird ? 'rgba(248, 113, 113, 0.7)' : 'rgba(29, 218, 255, 0.7)';
            const radius = isThird ? 16 : 12;
            const textClass = isThird ? 'fill-white font-extrabold text-xl' : 'fill-white font-bold text-base';
            const animationValues = isThird ? '15;17;15' : '11;13;11';
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                ...
                <div className="flex items-start space-x-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 leading-tight">Triff diesen Ton beim Akkordwechsel!</p>
                    </div>
                </div>
            </div>
        </div>
